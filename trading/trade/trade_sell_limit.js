...
appEmitter.prependListener('analyse:try_trade', async ({ market }) => {
    await intelligentTrade({ simulation: true });

    async function intelligentTrade({ simulation = true }) {
        ...
        async function startTrade() {
            ...
            if (amount) {
                if (!simulation) {
                    ...
                    //bid
                    buyOrder = await exchange.createLimitBuyOrder(symbol, amount, buyPrice, { "timeInForce": "FOK", });
        ...
        async function updateTrade() {
            ...
            if (gainOrLoss > MIN_GAIN_TO_CONTINUE_TRADE * (trade.update + 1)) {
                if (trade.simulation) {
                    endTrade();
                    return intelligentTrade({ simulation: false });
                }
                emit('updating', trade);
                //get the sell price
                let sellPrice = await updatePrice({ price: buyPrice, percent: SELL_LIMIT_PERCENT });
